...
func initConfigFileTo(target io.Writer, ui bool) error {
	...
	if err != nil {
		if errors.Is(err, os.ErrExist) {
			_, _ = fmt.Fprintf(target, "config already exists.\n%s\n", path)
			return nil
		}
	...
	_, _ = fmt.Fprintf(target, "config created.\n%s\n", path)
	return nil
}
...
func normalizeTruncationStrategy(value string, fallback core.TruncationStrategy) core.TruncationStrategy {
	...
	case string(core.TruncationStart):
		return core.TruncationStart
	case string(core.TruncationEnd):
		return core.TruncationEnd
	case string(core.TruncationMixed):
		return core.TruncationMixed
	default:
		return fallback
	}
}
